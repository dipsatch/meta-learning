...
local function processor(ind, opt, input)
   if not threadInitialized then
      ...
      function preprocessImages(images)
         return torch.cat(_.map(images, function(v, i)
   ...
   -- construct all image urls
    local urls = _.map(opt.imageFiles[class], function(v, i)
...
local function getData(opt)
   ...
   local splitDir = paths.concat(opt.dataCacheDir, '')
   local splits = {'train', 'val', 'test'}
   local splitFiles = { }
   _.each(splits, function(split, i)
